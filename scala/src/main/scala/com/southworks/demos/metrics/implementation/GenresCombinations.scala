...
class GenresCombinations extends MetricsSolver {
  override def run(moviesDataset: Dataset[Row], ratingsDataset: Dataset[Row]): Dataset[Row] = {
    ...
    val secondGenresList = firstGenresList
    ...
    val greatestCombinationByGenre = firstGenresList
